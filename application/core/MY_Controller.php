...
class Base_controller extends MX_Controller {
    ...
    protected $portal_judul = 'Marga Jaya Apps';
    ...
    public function _get_left_sidebar() {
        ...
        foreach ($this->m_site->get_parent_menu(array($this->group_id, 0, $this->portal_id)) as $dt_parent_menu) {
            ...
            $icon_menu = empty($dt_parent_menu['menu_fonticon']) ? '' : $dt_parent_menu['menu_fonticon'];
            if (!empty($child_menu)) {
                $html .= '<li ' . $child_menu['active'] . '>';
                $html .= '<a href="javascript:void(0);" title="' . $dt_parent_menu['menu_desc'] . '" class="">';
                $html .= '<i class="far ' . $icon_menu . '"></i> <span class="nav-link-text">' . $dt_parent_menu['menu_name'] . '<span class="far arrow"></span></span>';
                $html .= '</a>';
                $html .= $child_menu['html'];
                $html .= '</li>';
            } else {
                // cek apakah menu active atau tidak
                ...
                $html .= '<li ' . $class_stats . '><a class="has-arrow waves-effect waves-dark" href="' . base_url($dt_parent_menu['menu_link']) . '"><i class="far ' . $icon_menu . '"></i><span class="hide-menu"> ' . $dt_parent_menu['menu_name'] . '</span></a></li>';
            }
        }
        // echo "<pre>";
        // print_r($html);
    ...
    public function _get_child_menu_sidebar($parent_id) {
        ...
        if (!empty($list_child)) {
            ...
            foreach ($list_child as $dt_child_menu) {
                $icon_menu = empty($dt_child_menu['menu_fonticon']) ? '' : 'far ' . $dt_child_menu['menu_fonticon'];
                if ($this->uri->segment(1) == $dt_child_menu['menu_link'] || $this->uri->segment(1) . '/' . $this->uri->segment(2) == $dt_child_menu['menu_link']) {
                    $stat['active'] = 'class="active open"';
                    $class_stats = 'class="active"';
                } else {
                    $class_stats = 'class=""';
                }
                $stat['html'] .= '<li ' . $class_stats . '><a ' . $class_stats . ' href="' . base_url($dt_child_menu['menu_link']) . '" title="' . $dt_child_menu['menu_desc'] . '"><i class="' . $icon_menu . '"></i> ' . $dt_child_menu['menu_name'] . '</a></li>';
            }
            $stat['html'] .= '</ul>';
        }
        return $stat;
    ...
    public function _build_breadcrumb($par = null) {
        ...
        if ($dt_menu['menu_parent'] != 0) {
            ...
            $icon_menu = empty($dt_parent['menu_fonticon']) ? 'far far-view-sequential' : $dt_parent['menu_fonticon'];
            $html .= '<li class="breadcrumb-item"><a href="javascript:void(0);"><i class="far ' . $icon_menu . '"></i> ' . $dt_parent['menu_name'] . '</a></li>';
            $html .= '<li class="breadcrumb-item"><a href="' . $dt_menu['menu_link'] . '">' . $dt_menu['menu_name'] . '</a></li>';
            if (!empty($par)) {
                foreach ($par as $key => $vpar) {
                    $html .= '<li class="breadcrumb-item"><a href="' . $vpar . '">' . $key . '</a></li>';
                }
            }
        } else {
            $icon_menu = empty($dt_menu['menu_fonticon']) ? 'far' : $dt_menu['menu_fonticon'];
            $html .= '<li class="breadcrumb-item"><a href="javascript:void(0);"><i class="far ' . $icon_menu . '"></i> ' . $dt_menu['menu_name'] . '</a></li>';
            if (!empty($par)) {
                foreach ($par as $key => $vpar) {
                    $html .= '<li class="breadcrumb-item"><a href="' . $vpar . '">' . $key . '</a></li>';
                }
...
/*
 * Controller untuk portal admin
 */

class Portal_admin extends Base_controller {

    public function __construct() {
        parent::__construct();
        $this->portal_id = 1;
        $this->portal_judul = 'BIO Multicommerce :: Admin Backoffice';
    }
